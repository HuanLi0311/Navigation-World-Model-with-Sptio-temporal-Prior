...
class Attention(nn.Module):
    ...
    def eviction(
        ...
        # 确保需要保留的候选tokens数量不超过实际候选tokens数量
        num_to_keep_from_candidates = min(num_to_keep_from_candidates, num_candidates)
        # 确保num_to_keep_from_candidates至少为1，避免topk错误
        num_to_keep_from_candidates = max(num_to_keep_from_candidates, 0)

        if num_to_keep_from_candidates == 0:
            # 如果不需要保留任何候选tokens，只返回anchor部分
            final_k = anchor_k
            final_v = anchor_v
            scores = None
        else:
