...
@torch.no_grad()
def model_forward_wrapper(all_models, curr_obs, curr_delta, num_timesteps, latent_size, device, num_cond, num_goals=1, rel_t=None, progress=False, vggt_model=None, vggt_config=None, use_kv_cache=False):
    ...
    with torch.amp.autocast('cuda', enabled=True, dtype=torch.bfloat16):
        ...
        kv_cache = None
        if use_kv_cache and vggt_model is not None and vggt_config is not None:
...
def generate_rollout(args, output_dir, rollout_fps, idxs, all_models, obs_image, gt_image, delta, num_cond, device, vggt_model=None, vggt_config=None, use_kv_cache=False):
    ...
    for i in range(gt_image.shape[1]):
        ...
        if args.gt:
            ...
            x_pred_pixels = model_forward_wrapper(all_models, curr_obs, curr_delta, rollout_stride, args.latent_size, num_cond=num_cond, num_goals=1, device=device, vggt_model=vggt_model, vggt_config=vggt_config, use_kv_cache=use_kv_cache)
...
def generate_time(args, output_dir, idxs, all_models, obs_image, gt_output, delta, secs, num_cond, device, vggt_model=None, vggt_config=None, use_kv_cache=False):
    ...
    for sec, timestep in zip(secs, eval_timesteps):
        ...
        if args.gt:
            ...
            x_pred_pixels = model_forward_wrapper(all_models, obs_image, curr_delta, timestep, args.latent_size, num_cond=num_cond, num_goals=1, device=device, vggt_model=vggt_model, vggt_config=vggt_config, use_kv_cache=use_kv_cache)
...
def main(args):
    ...
    if use_kv_cache:
        ...
        print(f"Loaded VGGT model for KV cache extraction")
            ...
            with torch.amp.autocast('cuda', enabled=True, dtype=torch.bfloat16):
                ...
                if args.eval_type == 'rollout':
                    for rollout_fps in args.rollout_fps_values:
                        ...
                        generate_rollout(args, curr_rollout_output_dir, rollout_fps, idxs, model_lst, obs_image, gt_image, delta, num_cond, device, vggt_model, vggt_config, use_kv_cache)
                    ...
                    generate_time(args, curr_time_output_dir, idxs, model_lst, obs_image, gt_image, delta, secs, num_cond, device, vggt_model, vggt_config, use_kv_cache)
